...
//!
//! Provides a resilient wrapper around PostgreSQL/TimescaleDB database operations.
//! Transitions through Closed -> Open -> HalfOpen -> Closed states to prevent cascading
//! failures when the primary database cluster is experiencing downtime or network partitions.
...
"#;
    ...
    async fn test_open_to_half_open_after_recovery_timeout() {
        let breaker = DbCircuitBreaker::new(CircuitBreakerConfig {
            failure_threshold: 1,
            recovery_timeout_ms: 10, // short timeout for testing
            retry_attempts: 1,
            ..Default::default()
        });
    ...
    async fn test_half_open_failure_reopens() {
        ...
        // In HalfOpen, probe fails -> should transition back to Open
        let res = breaker
            .execute(|| async { Err::<(), _>(sqlx::Error::RowNotFound) })
            .await;
